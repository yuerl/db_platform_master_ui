...
from myapp.models import Db_name, Db_account, Db_instance, Oper_log, Task, Incep_error_log, Pro_mgr_detail
...
# 将字符串变为格式化的tupe数组
def oracle_sqlcheck(sql, dbname):
	dbname = dbname
	collist = ['ID', 'stage', 'errlevel', 'stagestatus', 'errormessage', 'SQL', 'Affected_rows', 'sequence',
	           'backup_dbname', 'execute_time', 'sqlsha1']
	# list_sql = str(str(sql.strip()).replace('\n','')).split(';')
	list_sql = oracle_sqlfilter_ddldml(sql)
	type_list = []
	for i in range(len(list_sql)):
		if len(str((list_sql[i]).strip())) > 0:
			tupe_item = (
			long(i), u'CHECKED', 0L, u'Audit completed', u'None', str(list_sql[i]), 0L, u"'0_0_0'", u'None', u'0', u'')
			type_list.append(tupe_item)
	type_list = list(type_list)
	return type_list, collist, dbname


def oracle_sqlfilter(sql):
	sql_type = ['INSERT', 'UPDATE', 'DELETE', 'ALTER', 'CREATE']
	sql_return = []
	sql = str(sql).strip()
	sql = str(sql).replace('\r\n', '')
	if len(sql) > 0:
		list_sql = str(sql).split(';')
		sql_single = ""
		for i in range(len(list_sql)):
			temp = str(list_sql[i]).split(' ')
			flag = 0
			for m in range(len(sql_type)):
				if str(temp[0]).upper() == str(sql_type[m]):
					# if len(sql_single)>0:
					flag = 1
				# else:
				#     sql_single=sql_single+str(list_sql[i])+";"
			if flag == 0 and len(str(list_sql[i])) > 0:
				sql_single = sql_single + str(list_sql[i]) + ";"
			elif flag == 1:
				if len(sql_single) > 0:
					sql_return.append(sql_single)
					sql_single = str(list_sql[i]) + ";\r\n"
...
def oracle_sqlfilter_ddldml(sql):
	sql_type = ['INSERT', 'UPDATE', 'DELETE', 'ALTER', 'CREATE']
	sql_return = []
	...
	if len(sql) > 0:
		...
		for i in range(len(list_sql)):
			flag = 0
			for m in range(len(sql_type)):
				if str(list_sql[i]).upper().startswith(sql_type[m]):
					flag = 1
					break
			if flag==0:
				if len(str(list_sql[i]))>0:
					sql_single = sql_single+str(list_sql[i])+';\r\n'
			elif  flag==1:
				if len(sql_single)>0:
					sql_return.append(sql_single)
				sql_single=str(list_sql[i])+';\r\n'
			if i==len(list_sql)-1 and len(sql_single)>0:
				sql_return.append(sql_single)
...
def task_check(idnum, request):
	task = Task.objects.get(id=idnum)
	db_type_record = func.get_dbtype_bydbtag(task.dbtag)
	...
	if task.status != 'executed' and task.status != 'running' and task.status != 'executed failed':
		...
		if len(results) > 1:
			...
			if dbtype_flag == 'mysql':
				...
				results, col, dbname = oracle_sqlcheck(sql, dbname_flag)
			status = 'check passed'
			str_sql = ''
			for row in results:
				if (int(row[2]) != 0):
...
def get_task_list(dbtag, request, end, start):
	username = request.user.username
	if request.user.has_perm('myapp.can_admin_task'):
		if (dbtag == 'all'):
			task_list = Task.objects.filter(create_time__lte=end).filter(create_time__gte=start).order_by(
				"-create_time")[0:50]
		else:
			task_list = Task.objects.filter(dbtag=dbtag).filter(create_time__lte=end).filter(
				create_time__gte=start).order_by("-create_time")[0:50]
	else:
		if (dbtag == 'all'):
			task_list = Task.objects.filter(user=username).filter(create_time__lte=end).filter(
				create_time__gte=start).order_by("-create_time")[0:50]
		else:
			task_list = Task.objects.filter(dbtag=dbtag).filter(create_time__lte=end).filter(
				create_time__gte=start).filter(user=username).order_by("-create_time")[0:50]
	return task_list
...
def record_taskdml(request, sqltext, dbtag, specify, ifbackup, status):
	username = request.user.username
	# lastlogin = user.last_login+datetime.timedelta(hours=8)
	# create_time = datetime.datetime.now()+datetime.timedelta(hours=8)
	create_time = datetime.datetime.now()
...
# add script
def record_script(request, sqltext, dbtag, pro_id, sqlmemo, status, operator, detail_id, add_task, task_id):
	username = request.user.username
	# lastlogin = user.last_login+datetime.timedelta(hours=8)
	# create_time = datetime.datetime.now()+datetime.timedelta(hours=8)
	create_time = datetime.datetime.now()
	...
	if len(detail_id) > 0:
		...
		if add_task == "yes":
			status = "finished";
		myscript = Pro_mgr_detail.objects.filter(id=detail_id).update(user=username, sqltext=sqltext,
		                                                              update_time=update_time,
		                                                              dbtag=dbtag, status=status, pro_id_id=pro_id,
		                                                              sql_memo=sqlmemo, operator=operator,
		                                                              add_task=add_task)
		return myscript
	else:
		myscript = Pro_mgr_detail(user=username, sqltext=sqltext, create_time=create_time, update_time=update_time,
		                          dbtag=dbtag, status=status, pro_id_id=pro_id, sql_memo=sqlmemo, operator=operator,
		                          add_task=add_task, detail_id=task_id)
		myscript.save()
		return myscript
...
def rollback_sqllist(idnum):
	...
	if task.backup_status == 2:
		data = Incep_error_log.objects.filter(create_time=task.create_time).filter(
			finish_time=task.update_time).order_by("-myid")
		for i in data:
...
def get_single_rollback(backupDb, opid_time):
	sqllist = []
	tbNamesql = "select tablename from %s.$_$Inception_backup_information$_$ where opid_time='%s';" % (
	backupDb, opid_time)
	...
	tbName = results[0][0]
	backsql = "select rollback_statement from %s.%s where opid_time='%s' order by id desc" % (
	backupDb, tbName, opid_time)
	...
	if task.status == 'executed failed' or task.status == 'executed':
		...
		if text == '':
			try:
				...
				pc = prpcrypt()
				tar_passwd = pc.decrypt(tar_passwd)
...
def set_schetime(idnum, schetime):
	...
	if task.status == 'check passed':
		...
		return 'please check passed first'
