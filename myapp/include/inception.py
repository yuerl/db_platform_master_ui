...
def get_task_list(dbtag,request,end,start):
    ...
    if request.user.has_perm('myapp.can_admin_task'):
        if (dbtag=='all'):
            task_list = Task.objects.filter(create_time__lte=end).filter(create_time__gte=start).order_by("-create_time")[0:50]
        else:
            task_list = Task.objects.filter(dbtag=dbtag).filter(create_time__lte=end).filter(create_time__gte=start).order_by("-create_time")[0:50]
    else:
        if (dbtag=='all'):
            task_list = Task.objects.filter(user=username).filter(create_time__lte=end).filter(create_time__gte=start).order_by("-create_time")[0:50]
        else:
            task_list = Task.objects.filter(dbtag=dbtag).filter(create_time__lte=end).filter(create_time__gte=start).filter(user=username).order_by("-create_time")[0:50]
    return task_list
...
def record_script(request,sqltext,dbtag,pro_id,sqlmemo,status,operator,detail_id,add_task,task_id):
    ...
    if len(detail_id)>0:
        ...
        if add_task=="yes":
            status = "finished";
        myscript = Pro_mgr_detail.objects.filter(id=detail_id).update(user=username, sqltext=sqltext, update_time=update_time,
                                  dbtag=dbtag, status=status, pro_id_id=pro_id, sql_memo=sqlmemo, operator=operator,add_task=add_task)
        return myscript
    else:
        myscript = Pro_mgr_detail(user=username, sqltext=sqltext, create_time=create_time, update_time=update_time,
                                  dbtag=dbtag, status=status, pro_id_id=pro_id, sql_memo=sqlmemo, operator=operator,add_task=add_task,detail_id=task_id)
        myscript.save()
        return myscript
