...
from myapp.include import inception as incept,wechart
from celery import task
from mysql_gather import monitor,monitor_h
...
wechart_task_fail_mins=config.wechart_task_fail_mins
...
@task
def gather_slowsql():
    #all database slow sql
    objlist = func.get_mysql_hostlist('admin', 'incept')
    return ''
@task
def gather_mysql_info():
    monitor()
@task
def gather_mysql_info_h():
    monitor_h()
#20180523 return db compare result
@task
def mysql_db_diff(choosed_host0,choosed_host1):
    results0, col0, tar_dbname0 = meta.get_metadata(choosed_host0, 1)
    results1, col1, tar_dbname1 = meta.get_metadata(choosed_host1, 1)
    row_number = 0
    # list_result = []
    func.delete_all_mysql_db_diff(choosed_host0,choosed_host1)
    for item in results1:
        # list_item = {}
        new_flag = 0
        for item0 in results0:
            if item0[0] == item[0]:
                new_flag = 1
                result = func.get_diff(choosed_host0, item[0], choosed_host1, item[0])
                # ["host0":choosed_host0, "item0":item[0], "host1":choosed_host1, "item1":item[0], "result":result]
                result = result.replace("# WARNING: Using a password on the command line interface can be insecure.",
                                        "")
                if len(result.strip()) > 0:
                    row_number = row_number + 1
                    # list_item = [choosed_host0,item[0], choosed_host1, item[0],result]
                    # list_item = {"row_number": row_number, "stype": "Diff", "host0": choosed_host0, "item0": item[0],
                    #              "host1": choosed_host1, "item1": item[0], "result": result}
                    func.add_mysql_db_diff(row_number,"Diff",choosed_host0,item[0],choosed_host1,item[0],result,datetime.datetime.now())
                    # list_result.append(list_item)
        if new_flag == 0:
            (sh_cre1, sh_cre_col, dbname) = meta.get_metadata(choosed_host1, 5, item[0])
            row_number = row_number + 1
            # list_item = {"row_number": row_number, "stype": "Lost", "host0": choosed_host0, "item0": "",
            #              "host1": choosed_host1, "item1": item[0],
            #              "result": str(sh_cre1[0][1]).replace(
            #                  "# WARNING: Using a password on the command line interface can be insecure.", "").strip()}
            func.add_mysql_db_diff(row_number, "Lost", choosed_host0, "", choosed_host1, item[0], str(sh_cre1[0][1]).replace(
                             "# WARNING: Using a password on the command line interface can be insecure.", "").strip(),
                                   datetime.datetime.now())


@task
def task_failed_wechart():
    #发送微信,最近失败的任务
    mins=wechart_task_fail_mins
    mins=int(mins)
    ...
    for i in rows:
        try:
            ...
            wechart.wxsenddata(str(i[0]), '【Type】'+str(mins)+'Minutes Task Failed', "【Time】"+str(stime)+"\r\n"+'【Msg】Your Have '+str(i[2])+' Tasks Failed')
        except Exception, e:
            pass