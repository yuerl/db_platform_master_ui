...
class sqldForm (forms.Form):
    a = forms.CharField(widget=forms.Textarea(attrs={'cols': 200, 'rows': 5}))
...
class Scriptpromgr(forms.Form):
    begin = forms.DateTimeField(label='dateinfo')
    end = forms.DateTimeField(label='dateend')
...
class BeginEndTime(forms.Form):
     begin = forms.DateTimeField(label='datebegin')
     end =  forms.DateTimeField(label='dateend')
